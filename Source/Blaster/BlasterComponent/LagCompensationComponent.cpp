...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime)
{
	...
	TDoubleLinkedList<FFramePackage>::TDoubleLinkedListNode* Oldest =Younger;
	while (Oldest->GetValue().Time > HitTime)
	{
		if(Oldest->GetNextNode() == nullptr) break;
		Oldest = Oldest->GetNextNode();
		if(Oldest->GetValue().Time > HitTime)
		{
			Younger = Oldest;
		}
	}
	if(Oldest->GetValue().Time == HitTime)
	{
		FrameToCheck = Oldest->GetValue();
		bShouldInterpolate = false;
	}
	if(bShouldInterpolate)
	{
		// Interpolate beetwen young and old
		FrameToCheck = InterpBetweenFrames(Oldest->GetValue(),Younger->GetValue(),HitTime);
	}
	...
	const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	else // hit no head , check rest boxes
	{
	...
	ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	if(ConfirmHitResult.bBlockingHit) // we hit a head
	{
	...
	else // didnt hit a head, check the rest of boxes
	{
		for(auto& HitBoxPair : HitCharacter->HitCollisionBoxes)
		{
			if(HitBoxPair.Value != nullptr)
